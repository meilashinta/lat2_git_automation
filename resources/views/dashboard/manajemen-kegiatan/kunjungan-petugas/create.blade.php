                    ...
                    <form role="form" action="/dashboard-kunjungan-petugas" method="POST">
                        ...
                        <div class="card-body">
                            <div class="form-group row">
                                <label for="jadwal_kunjungan_id" class="col-sm-2 col-form-label">Jadwal Kunjungan</label>
                                <div class="col-sm-10">
                                    ...
                                    <select name="jadwal_kunjungan_id" class="form-control">
                                        @foreach ($jadwalKunjungan as $item)
                                            ...
                                            <option value="{{ $item->id }}">
                                                {{ $item->user->name}} | Tanggal Kunjungan: {{ $tanggalKunjungan }} | Jam Mulai: {{ $jamMulai }} | Jam Selesai: {{ $jamSelesai }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-sm-10">
                                    @foreach ($pegawai as $item)
                                                ...
                                                <div class="form-check">
                                                    <input type="checkbox" name="pegawai_id[]" value="{{ $item->id }}" class="form-check-input" id="pegawai_{{ $item->id }}">
                                                    <label class="form-check-label" for="pegawai_{{ $item->id }}">{{ $item->nama_pegawai }}</label>
                                                </div>
